...
| `lens` | `fn` `var` `arg` | | 要顯示的代碼透鏡。`fn` — 函式/native 上方的參考計數，`var` — 變數（全域+區域）上方的參考計數，`arg` — 函式參數上方的參考計數。無指令時不顯示代碼透鏡。 |
...
| `build-opts` | `uglify` `nolocal` | | 建構選項標籤。`uglify` 啟用建構輸出中的標識符壓縮。`nolocal` 會將「回傳局部變數」的洩漏修復切換為無局部 temp 的策略（使用暫時全域變數，而不是引入暫時區域變數）。一條指令中可以組合多個標籤。 |
| `build-uglify` | `0 \| 1` | `0` | `//set build-opt uglify` 的舊別名。為相容性保留。 |
