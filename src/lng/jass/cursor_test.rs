...
";
    ...
    fn semantic_no_trailing_newline() {
        ...
        with_cursor(src, |c| {
            let data = c.semantic.data(None);
            assert!(!data.is_empty(), "Should have semantic tokens even without trailing newline");
            ...
            let int_tok = line1.unwrap().tokens.iter().find(|t| t.col == 4 && t.len == 7);
            assert!(
                int_tok.is_some(),
    ...
    fn semantic_no_trailing_newline_single_line() {
        ...
        let src = "globals\ninteger a = 0xFF23A284\nendglobals";
        with_cursor(src, |c| {
            eprintln!("All semantic lines: {:?}", c.semantic.lines.keys().collect::<Vec<_>>());
