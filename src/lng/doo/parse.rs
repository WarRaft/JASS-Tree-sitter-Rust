//! Binary reader for Warcraft III placement files (`.doo`).
//!
//! The `.doo` format describes unit placement ("war3mapUnits.doo") and
//! doodad/destructible placement ("war3map.doo").  The format is described
//! in `doo.hexpat` (ImHex pattern) and at
//! <https://xgm.guru/p/wc3/w3-file-format>.
//!
//! All multi-byte integers are **little-endian**.

use crate::util::bin_reader::{BinRead, BinReader, BinReaderMeta, BinResult, Rawcode};
use serde::Serialize;

// ─── Simple types ────────────────────────────────────────────────────────────

crate::bin_struct! {
    /// 3-D vector (hexpat `struct Vector`).
    pub Vector { x: f32, y: f32, z: f32 }
}

crate::bin_enum! {
    /// Doodad visibility / collision flag (hexpat `enum Flag : u8`).
    pub Flag: u8 {
        Invisible = 0,
        Visible = 1,
        Normal = 2,
    }
}

crate::bin_struct! {
    /// A single dropped item entry (hexpat `struct DropItem`).
    pub DropItem { rawcode: Rawcode, chance: u32 }
}

crate::bin_struct! {
    /// A cliff / terrain decoration (hexpat `struct Cliff`).
    pub Cliff { rawcode: Rawcode, variation: u32, x: u32, y: u32 }
}

crate::bin_struct! {
    /// An item in a hero's inventory (hexpat `struct InventoryItem`).
    pub InventoryItem { slot: u32, rawcode: Rawcode }
}

crate::bin_struct! {
    /// A modified ability on a placed unit (hexpat `struct ModifiedAbility`).
    pub ModifiedAbility { rawcode: Rawcode, autocast: u32, level: u32 }
}

crate::bin_struct! {
    /// One entry in a random-unit table (hexpat `struct RandomUnit`).
    pub RandomUnit { rawcode: Rawcode, chance: u32 }
}

// ─── Drop set (counted Vec) ─────────────────────────────────────────────────

/// One set of droppable items (hexpat `struct Drop`).
#[derive(Debug, Clone, Serialize)]
pub struct Drop {
    pub items: Vec<DropItem>,
}

impl BinRead for Drop {
    fn bin_read(r: &mut BinReader) -> BinResult<Self> {
        Ok(Self { items: r.read_vec()? })
    }
}

// ─── Random data (tagged union) ──────────────────────────────────────────────

/// The `randomType` field determines which variant is stored.
#[derive(Debug, Clone, Serialize)]
#[serde(tag = "kind")]
pub enum RandomData {
    /// `randomType == 0` — level + item class encoded in 4 bytes.
    Level {
        /// 24-bit level value (-1 = any).
        random_level: [u8; 3],
        /// Item class (0 = any, 1–7 for iDNR; 0 for others).
        item_class: u8,
    },
    /// `randomType == 1` — references a random group from `war3map.w3i`.
    Group {
        random_group: u32,
        random_position: u32,
    },
    /// `randomType == 2` — inline table of random units.
    Table {
        units: Vec<RandomUnit>,
    },
}

// ─── UnitExtra (all unit-specific fields) ────────────────────────────────────

/// Data specific to a **unit** placement.
#[derive(Debug, Clone, Serialize)]
pub struct UnitExtra {
    pub player: u32,
    pub unknown1: u8,
    pub unknown2: u8,
    /// Hit points.  -1 = default.
    pub health: u32,
    /// Mana points.  -1 = default.
    pub mana: u32,
    /// Drop-table index (subformat ≥ 11).  -1 = not used.
    pub drop_table: Option<i32>,
    /// Item drop sets.
    pub drops: Vec<Drop>,
    /// Gold in mine.  Default 12500.
    pub gold_mine: i32,
    /// Acquisition range.  -1.0 = normal, -2.0 = camp.
    pub target: f32,
    /// Hero level (1 for non-heroes / items).
    pub hero_level: u32,
    /// Hero Strength  (subformat ≥ 11).  0 = default.
    pub strength: Option<u32>,
    /// Hero Agility   (subformat ≥ 11).  0 = default.
    pub agility: Option<u32>,
    /// Hero Intelligence (subformat ≥ 11).  0 = default.
    pub intelligence: Option<u32>,
    /// Hero inventory items.
    pub inventory: Vec<InventoryItem>,
    /// Modified abilities.
    pub abilities: Vec<ModifiedAbility>,
    /// Random-unit data (tagged by `random_type`).
    pub random_type: i32,
    pub random_data: Option<RandomData>,
    /// Custom player colour.  -1 = none.
    pub custom_color: i32,
    /// Waygate target region in `war3map.w3r`.  -1 = none.
    pub waygate: i32,
    /// Creation number (unique object id).
    pub object_id: u32,
}

impl UnitExtra {
    pub(crate) fn read(r: &mut BinReader, subformat: u32) -> BinResult<Self> {
        let player = r.read_u32()?;
        let unknown1 = r.read_u8()?;
        let unknown2 = r.read_u8()?;
        let health = r.read_u32()?;
        let mana = r.read_u32()?;

        let drop_table = if subformat >= 11 {
            Some(r.read_s32()?)
        } else {
            None
        };

        let drops: Vec<Drop> = r.read_vec()?;

        let gold_mine = r.read_s32()?;
        let target = r.read_f32()?;
        let hero_level = r.read_u32()?;

        let (strength, agility, intelligence) = if subformat >= 11 {
            (Some(r.read_u32()?), Some(r.read_u32()?), Some(r.read_u32()?))
        } else {
            (None, None, None)
        };

        let inventory: Vec<InventoryItem> = r.read_vec()?;
        let abilities: Vec<ModifiedAbility> = r.read_vec()?;

        let random_type = r.read_s32()?;
        let random_data = match random_type {
            0 => {
                let b = r.read_bytes(3)?;
                let random_level = [b[0], b[1], b[2]];
                let item_class = r.read_u8()?;
                Some(RandomData::Level { random_level, item_class })
            }
            1 => {
                let random_group = r.read_u32()?;
                let random_position = r.read_u32()?;
                Some(RandomData::Group { random_group, random_position })
            }
            2 => {
                let units: Vec<RandomUnit> = r.read_vec()?;
                Some(RandomData::Table { units })
            }
            _ => None,
        };

        let custom_color = r.read_s32()?;
        let waygate = r.read_s32()?;
        let object_id = r.read_u32()?;

        Ok(Self {
            player, unknown1, unknown2, health, mana,
            drop_table, drops, gold_mine, target, hero_level,
            strength, agility, intelligence,
            inventory, abilities,
            random_type, random_data,
            custom_color, waygate, object_id,
        })
    }
}

// ─── DoodadExtra (conditional fields) ────────────────────────────────────────

/// Data specific to a **doodad / destructible** placement.
#[derive(Debug, Clone, Serialize)]
pub struct DoodadExtra {
    pub health: u8,
    pub drop_index: Option<i32>,
    pub drops: Option<Vec<Drop>>,
    pub num: u32,
}

// ─── Placed item (unit or doodad) ────────────────────────────────────────────

/// A single placed object (unit or doodad/destructible).
#[derive(Debug, Clone, Serialize)]
pub struct DooItem {
    pub rawcode: Rawcode,
    pub variation: u32,
    pub position: Vector,
    /// Angle in radians.
    pub angle: f32,
    pub scale: Vector,
    /// Skin override (patch ≥ 32).
    pub skin: Option<u32>,
    pub flag: Flag,
    /// `Some(UnitExtra)` when parsing a units file, `None` for doodads.
    pub unit: Option<UnitExtra>,
    /// `Some(DoodadExtra)` when parsing a doodads file, `None` for units.
    pub doodad: Option<DoodadExtra>,
}

impl DooItem {
    fn read(r: &mut BinReader, is_unit: bool, format: u32, subformat: u32, patch: u32) -> BinResult<Self> {
        let rawcode = Rawcode::bin_read(r)?;
        let variation = r.read_u32()?;
        let position = Vector::bin_read(r)?;
        let angle = r.read_f32()?;
        let scale = Vector::bin_read(r)?;

        let skin = if patch >= 32 { Some(r.read_u32()?) } else { None };
        let flag = Flag::bin_read(r)?;

        let (unit, doodad) = if is_unit {
            (Some(UnitExtra::read(r, subformat)?), None)
        } else {
            let health = r.read_u8()?;
            let (drop_index, drops) = if format == 8 {
                let drop_index = r.read_s32()?;
                let drops = r.read_vec()?;
                (Some(drop_index), Some(drops))
            } else {
                (None, None)
            };
            let num = r.read_u32()?;
            (None, Some(DoodadExtra { health, drop_index, drops, num }))
        };

        Ok(Self { rawcode, variation, position, angle, scale, skin, flag, unit, doodad })
    }
}

// ─── Top-level DOO data ──────────────────────────────────────────────────────

/// Parsed contents of a `.doo` file.
#[derive(Debug, Clone, Serialize)]
pub struct DooData {
    pub magic: String,
    pub format: u32,
    pub subformat: u32,
    pub items: Vec<DooItem>,
    /// Cliff decorations — only present in doodad (non-unit) files.
    pub cliffs: Option<Vec<Cliff>>,
}

impl DooData {
    /// Parse a `.doo` file from raw bytes.
    ///
    /// * `is_unit` — `true` for `war3mapUnits.doo`, `false` for `war3map.doo`.
    /// * `patch`   — patch version from the parent `.w3i` (affects field layout).
    pub fn read(data: &[u8], is_unit: bool, patch: u32) -> BinResult<(Self, BinReaderMeta)> {
        let mut r = BinReader::new(data);

        let magic = r.read_fixed_string(4)?;  // "W3do"
        let format = r.read_u32()?;           // ROC=7, TFT=8
        let subformat = r.read_u32()?;

        let item_count = r.read_u32()?;
        let mut items = Vec::with_capacity(item_count as usize);
        for _ in 0..item_count {
            items.push(DooItem::read(&mut r, is_unit, format, subformat, patch)?);
        }

        let cliffs = if !is_unit {
            let _cliff_version = r.read_u32()?;
            let cliffs = r.read_vec()?;
            Some(cliffs)
        } else {
            None
        };

        let meta = r.meta();
        Ok((DooData { magic, format, subformat, items, cliffs }, meta))
    }
}

