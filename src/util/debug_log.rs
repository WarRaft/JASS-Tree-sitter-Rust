...
/// 4096 slots — if no client is connected the messages are silently dropped.
/// Large buffer to prevent message loss during heavy operations like decorations build.
static TX: Lazy<broadcast::Sender<String>> = Lazy::new(|| broadcast::channel(4096).0);
